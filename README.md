...
- 외부 플랫폼 13개의 `초안·운영 게이트·보류·후순위` 상태 표시
- 채널 원고 12종 생성
  ...
  - Show HN 사람 검토용 작업본
- 원고별 편집·복사·Markdown 저장, 12종 일괄 다운로드
- 마지막 작업을 브라우저 `localStorage`에 저장하고 이전 3종 데이터는 v2 형식으로 마이그레이션
- 자동 게시, 예약 게시, OAuth, SNS API 쓰기 연동 없음
- 외부 패키지, LLM API, GitHub token의 브라우저 저장 없음

TikTok·Discord·Bluesky·Mastodon은 상태만 표시하는 후순위 또는 선택 채널입니다. 모든 원고가 생성되어도 동시에 게시하지 않고, 운영 게이트를 확인한 채널부터 한 번에 하나씩 게시합니다.
...
4. 12개 탭의 원고를 검토·수정한 뒤 복사하거나 Markdown으로 저장합니다.
