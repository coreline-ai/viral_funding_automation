...
test("정적 파일만 allowlist로 제공하고 보안 헤더를 설정한다", async () => {
  await withServer({}, async (origin) => {
    ...
    const xPreview = await fetch(`${origin}/x-preview.mjs`);
    assert.equal(xPreview.status, 200);
    assert.match(xPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await xPreview.text(), /createXPreviewModel/);
    const linkedinPreview = await fetch(`${origin}/linkedin-preview.mjs`);
    assert.equal(linkedinPreview.status, 200);
    assert.match(linkedinPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await linkedinPreview.text(), /createLinkedInPreviewModel/);
    const facebookPreview = await fetch(`${origin}/facebook-preview.mjs`);
    assert.equal(facebookPreview.status, 200);
    assert.match(facebookPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await facebookPreview.text(), /createFacebookPreviewModel/);
    const instagramPreview = await fetch(`${origin}/instagram-preview.mjs`);
    assert.equal(instagramPreview.status, 200);
    assert.match(instagramPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await instagramPreview.text(), /createInstagramPreviewModel/);
    const shortsPreview = await fetch(`${origin}/shorts-preview.mjs`);
    assert.equal(shortsPreview.status, 200);
    assert.match(shortsPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await shortsPreview.text(), /createShortsPreviewModel/);
    const productHuntPreview = await fetch(`${origin}/product-hunt-preview.mjs`);
    assert.equal(productHuntPreview.status, 200);
    assert.match(productHuntPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await productHuntPreview.text(), /createProductHuntPreviewModel/);
    const peerlistPreview = await fetch(`${origin}/peerlist-preview.mjs`);
    assert.equal(peerlistPreview.status, 200);
    assert.match(peerlistPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await peerlistPreview.text(), /createPeerlistPreviewModel/);
    const disquietPreview = await fetch(`${origin}/disquiet-preview.mjs`);
    assert.equal(disquietPreview.status, 200);
    assert.match(disquietPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await disquietPreview.text(), /createDisquietPreviewModel/);
    const redditPreview = await fetch(`${origin}/reddit-preview.mjs`);
    assert.equal(redditPreview.status, 200);
    assert.match(redditPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await redditPreview.text(), /createRedditPreviewModel/);
    const indieHackersPreview = await fetch(`${origin}/indie-hackers-preview.mjs`);
    assert.equal(indieHackersPreview.status, 200);
    assert.match(indieHackersPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await indieHackersPreview.text(), /createIndieHackersPreviewModel/);
    const devPreview = await fetch(`${origin}/dev-preview.mjs`);
    assert.equal(devPreview.status, 200);
    assert.match(devPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await devPreview.text(), /createDevPreviewModel/);
    const okkyPreview = await fetch(`${origin}/okky-preview.mjs`);
    assert.equal(okkyPreview.status, 200);
    assert.match(okkyPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await okkyPreview.text(), /createOkkyPreviewModel/);
    const geeknewsPreview = await fetch(`${origin}/geeknews-preview.mjs`);
    assert.equal(geeknewsPreview.status, 200);
    assert.match(geeknewsPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await geeknewsPreview.text(), /createGeekNewsPreviewModel/);
    const showHnPreview = await fetch(`${origin}/show-hn-preview.mjs`);
    assert.equal(showHnPreview.status, 200);
    assert.match(showHnPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await showHnPreview.text(), /createShowHnPreviewModel/);
    const tiktokPreview = await fetch(`${origin}/tiktok-preview.mjs`);
    assert.equal(tiktokPreview.status, 200);
    assert.match(tiktokPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await tiktokPreview.text(), /createTikTokPreviewModel/);
    const discordPreview = await fetch(`${origin}/discord-preview.mjs`);
    assert.equal(discordPreview.status, 200);
    assert.match(discordPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await discordPreview.text(), /createDiscordPreviewModel/);
    const blueskyPreview = await fetch(`${origin}/bluesky-preview.mjs`);
    assert.equal(blueskyPreview.status, 200);
    assert.match(blueskyPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await blueskyPreview.text(), /createBlueskyPreviewModel/);
    const mastodonPreview = await fetch(`${origin}/mastodon-preview.mjs`);
    assert.equal(mastodonPreview.status, 200);
    assert.match(mastodonPreview.headers.get("content-type"), /^text\/javascript/);
    assert.match(await mastodonPreview.text(), /createMastodonPreviewModel/);
    const previewRegistry = await fetch(`${origin}/platform-preview-registry.mjs`);
    assert.equal(previewRegistry.status, 200);
    assert.match(previewRegistry.headers.get("content-type"), /^text\/javascript/);
    assert.match(await previewRegistry.text(), /PLATFORM_PREVIEW_REGISTRY/);
